...
def resolve_image_path(image_arg: str) -> tuple[Path, str, Path]:
    """
    ...
    Defaults:
        image:    data/chapters-long/<CH>.png
    ...
    if p.exists():
        ...
        stem = p.stem
        image_path = Path("data") / "chapters-long" / p.name
    else:
        stem = image_arg
        image_path = Path("data") / "chapters-long" / f"{stem}.png"
    ...
    if not image_path.exists():
        raise FileNotFoundError(
            ...
            f"Run from project root or pass an explicit image path."
        )

    return image_path, stem, temp_dir


def resolve_roi_path(rois_arg: str | None, stem: str, temp_dir: Path) -> Path:
    """
    Default ROI lookup order:
        1. --rois path
        2. data/temp/<CH>/used_rois_<CH>_boundary.txt
        3. newest data/temp/<CH>/used_rois*boundary*.txt
        4. newest data/temp/<CH>/used_rois*.txt
    """
    if rois_arg:
        path = Path(rois_arg)
        if not path.exists():
    ...
    preferred = temp_dir / f"used_rois_{stem}_boundary.txt"

    if preferred.exists():
        return preferred

    candidates = sorted(
        temp_dir.glob("used_rois*boundary*.txt"),
    ...
    if not candidates:
        candidates = sorted(
            temp_dir.glob("used_rois*.txt"),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        )

    if candidates:
        print(f"Using ROI file: {candidates[0]}")
        return candidates[0]

    raise FileNotFoundError(
        f"ROI file was not found in: {temp_dir}\n"
        f"Expected: {preferred}\n"
        f"Generate it first, for example:\n"
        f"  python tools/mask_boundary_roi.py {stem}"
    )


def default_roi_output_path(stem: str, temp_dir: Path) -> Path:
...
def _resolve_morph_orders(requested: str, default_order: str) -> list[str]:
    if requested not in {"auto", "minmax", "maxmin", "both"}:
        raise ValueError("--morph-order must be auto, minmax, maxmin or both")
...
def get_search_profile(profile_name: str, wide: bool, morph_order: str) -> SearchProfile:
    profile_name = profile_name.strip().lower()

    # Backward-compatible aliases.
    # Recommended usage is explicit:
    #   black-hard, black-soft, white-hard, white-soft
    alias_map = {
        "hard": "white-hard",
        "soft": "white-soft",
        "black-bg": "black-hard",
    ...
    if profile_name == "white-hard":
        default_order = "minmax"

        return SearchProfile(
            profile_name="white-hard",
            background="white",
            mask_kind="hard",
            black_weight=4.0,
            white_weight=1.5,
            ...
            if not wide
            else list(range(14, 49, 2)),
            level_white_values=[70, 80, 90, 100, 110, 120, 130, 150]
            if not wide
            else list(range(60, 171, 5)),
            gamma_x100_values=[30, 35, 40, 45, 48, 55, 65, 80]
            if not wide
            else [25, 30, 35, 40, 45, 50, 55, 65, 80, 100],
            threshold_values=[20, 24, 28, 29, 32, 36, 40, 48, 56, 64]
            if not wide
            else [16, 20, 24, 28, 29, 32, 36, 40, 48, 56, 64, 72, 80],
            min_radius_values=[10, 12, 14, 16, 18, 20, 22]
            if not wide
            else [8, 10, 12, 14, 16, 18, 20, 22, 24],
            max_radius_values=[10, 12, 14, 16, 18, 20, 22]
            if not wide
            else [8, 10, 12, 14, 16, 18, 20, 22, 24],
            morph_orders=_resolve_morph_orders(morph_order, default_order),
        )

    if profile_name == "white-soft":
        default_order = "minmax"

        return SearchProfile(
            profile_name="white-soft",
            background="white",
            mask_kind="soft",
            black_weight=1.0,
            white_weight=4.0,
    ...
    if profile_name == "black-hard":
        default_order = "maxmin"

        return SearchProfile(
            profile_name="black-hard",
            background="black",
            mask_kind="hard",
            black_weight=4.0,
            white_weight=5.0,
    ...
    if profile_name == "black-soft":
        default_order = "maxmin"

        return SearchProfile(
            profile_name="black-soft",
            background="black",
            mask_kind="soft",
            black_weight=2.0,
            white_weight=6.0,
            channels=["grayscale", "max RGB", "value", "saturation"],
            level_black_values=[0, 4, 8, 12, 16, 20, 24, 30, 36, 42, 50]
            if not wide
            else list(range(0, 73, 2)),
            level_white_values=[50, 65, 80, 95, 110, 130, 150, 175, 200]
            if not wide
            else list(range(40, 211, 5)),
            gamma_x100_values=[40, 50, 60, 70, 80, 100, 120, 150]
            if not wide
            else [30, 40, 50, 60, 70, 80, 100, 120, 150, 180],
            threshold_values=[8, 12, 16, 20, 24, 28, 32, 40, 48, 56, 64, 80]
            if not wide
            else [8, 12, 16, 20, 24, 28, 32, 36, 40, 48, 56, 64, 80, 96, 112],
            min_radius_values=[0, 1, 2, 3, 4, 6, 8, 10, 12]
            if not wide
            else [0, 1, 2, 3, 4, 6, 8, 10, 12, 16],
    ...
    raise ValueError("--profile must be black-hard, black-soft, white-hard or white-soft")
...
def search_stage1_tonal(rgb, rois, profile) -> list[TonalParams]:
    ...
    total = len(combos)
    results = []
    print(f"\nStage 1: Levels + Threshold [{profile.profile_name}], combinations={total}")
    start = time.time()
...
def save_best_settings(out_dir: Path, best: FinalParams, profile: SearchProfile) -> None:
    with (out_dir / "best_settings.txt").open("w", encoding="utf-8") as f:
        f.write(f"profile: {profile.profile_name}\n")
        f.write(f"background: {profile.background}\n")
        f.write(f"mask_kind: {profile.mask_kind}\n")
...
def save_all(image_path: Path, rgb, rois, stage1, final, profile, out_dir: Path) -> None:
    ...
    print(f"  mask_kind: {profile.mask_kind}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--rois",
        default=None,
        help="ROI file. Default: newest/actual used_rois*.txt from data/temp/<CH>.",
    )
    parser.add_argument(
        "--mark-rois",
        action="store_true",
        help="Open interactive ROI marker instead of using current ROIs from temp.",
    )
    parser.add_argument(
        "--roi-output",
        default=None,
    ...
    parser.add_argument(
        ...
        default="auto",
        help="auto=default for selected profile; minmax=Minimum then Maximum; maxmin=Maximum then Minimum; both=test both",
    )
    ...
    if args.mark_rois:
        ...
        roi_path = resolve_roi_path(args.rois, stem, temp_dir)
        rois = load_rois(roi_path)
