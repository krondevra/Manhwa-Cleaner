...
def resolve_paths(input_arg: str) -> tuple[Path, Path, str]:
    p = Path(input_arg)
    ...
    if p.suffix.lower() == ".png":
        ...
        stem = input_arg
        original_path = Path(f"{stem}.png")

    cleaned_path = Path(f"{stem}_cleaned.png")
    ...
    return original_path, cleaned_path, stem
...
def main() -> None:
    ...
    parser.add_argument(
        "image",
        help="Base name or original image path. Example: 033 or 033.png",
    )

    parser.add_argument(
        "--cleaned",
        default=None,
        help="Optional cleaned image path. Default: <stem>_cleaned.png",
    )

    parser.add_argument(
        "--output",
        default=None,
        help="Output ROI file. Default: used_rois_<stem>_boundary.txt",
    )
    ...
    original_path, default_cleaned_path, stem = resolve_paths(args.image)

    cleaned_path = Path(args.cleaned) if args.cleaned else default_cleaned_path

    if not cleaned_path.exists():
        raise FileNotFoundError(f"Cleaned image not found: {cleaned_path}")

    output_path = Path(args.output) if args.output else Path(f"used_rois_{stem}_boundary.txt")
    ...
    preview_path = Path(f"{stem}_boundary_rois_preview.jpg")
    boundary_path = Path(f"{stem}_boundary_debug.jpg")
    stats_path = Path(f"{stem}_boundary_stats.txt")
    ...
    print(
        f"python ThresholdGUI-Auto-darkbg-v2.py {original_path.name} "
        f"--rois {output_path.name} "
        f"--priority black-bg "
        f"--morph-order both "
        f"--top-tonal 60"
    )
