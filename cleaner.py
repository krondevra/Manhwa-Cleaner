...
def train_command(args: argparse.Namespace) -> None:
    ...
    samples = load_samples(Path(args.samples), args.alpha_threshold, guidance_params)
...
def process_command(args: argparse.Namespace) -> None:
    device = choose_device(args.device)
    log(f"device: {device}")
    model, config = load_model(Path(args.model), device)
...
def process_folder_command(args: argparse.Namespace) -> None:
    ...
    files = [
        ...
        if p.is_file() and p.suffix.lower() in VALID_EXTENSIONS and not p.stem.endswith(CLEAN_SUFFIX)
    ]
    if not files:
...
def add_inference_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--model", default="models/r_cleaner.pt")
    ...
    parser.add_argument("--no-red-preview", action="store_true")
    parser.add_argument("--threshold-value", type=int, default=90)
    parser.add_argument("--morph-radius", type=int, default=2)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manhwa cleaner. Learns full delete mask from original + cleaned pages.")
    sub = parser.add_subparsers(dest="command", required=True)

    p_train = sub.add_parser("train", help="Train from samples/* + samples/*_cleaned.png pairs.")
    p_train.add_argument("--samples", default="samples")
    p_train.add_argument("--model", default="models/r_cleaner.pt")
    ...
    p_folder = sub.add_parser("process-folder", help="Clean all chapters from a folder")
    p_folder.add_argument("--input", default="chapters")
    p_folder.add_argument("--output", default="chapters-results")
